          ...
          {viagens.length === 0 ? (
            ...
            viagens.map((viagem) => {
              ...
              return (
                <div
                  key={viagem.id}
                  className={`bg-slate-900 border ${estourou ? "border-rose-500/50" : "border-slate-800"} rounded-2xl p-6 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 transition-all`}
                >
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-3">
                      {/* Mapeado para os campos do seu DTO real */}
                      <h3 className="text-lg font-bold text-white">
                        {viagem.solicitacaoVeiculo ||
                    ...
                    <div className="w-full md:w-auto text-right p-4">
                      {estourou ? (
                        <p className="text-xs text-rose-400 font-bold max-w-[200px]">
                          Alerta de Estouro: O orçamento planejado estourou em{" "}
