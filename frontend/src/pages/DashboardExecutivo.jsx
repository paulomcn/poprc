...
import { Briefcase, DollarSign, CheckCircle, AlertTriangle, Plane, Percent, TrendingUp, Filter, Calendar } from 'lucide-react'
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 flex items-center gap-2">
            <TrendingUp className="text-slate-700" size={32} /> Dashboard Executivo
          </h1>
          <p className="text-slate-600 mt-1">Visão analítica macro do sistema</p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-white p-2 rounded-lg border shadow-sm w-fit">
          <span className="text-xs font-bold text-slate-500 uppercase px-2">Visualizar como:</span>
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 flex items-center justify-between">
          <div className="space-y-2">
            <p className="text-sm font-semibold text-slate-500 uppercase">Contratos Ativos</p>
            <h3 className="text-3xl font-bold text-slate-800">{data?.contratosAtivos}</h3>
          </div>
          <div className="p-4 bg-slate-100 text-slate-700 rounded-lg"><Briefcase size={24} /></div>
        </div>

        {/* Card 2: Valor Global (Visível para Ambos) */}
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 flex items-center justify-between">
          <div className="space-y-2">
            <p className="text-sm font-semibold text-slate-500 uppercase">Total Alocado</p>
            <h3 className="text-2xl font-bold text-slate-800">{formatCurrency(data?.valorTotalContratado)}</h3>
          </div>
          <div className="p-4 bg-green-50 text-green-700 rounded-lg"><DollarSign size={24} /></div>
        </div>
        ...
        {userRole === 'DIRETORIA' ? (
          <>
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 flex items-center justify-between">
              <div className="space-y-2">
                <p className="text-sm font-semibold text-slate-500 uppercase">Faturado Global RC</p>
                <h3 className="text-2xl font-bold text-green-600">{formatCurrency(data?.valorFaturado)}</h3>
              </div>
              <div className="p-4 bg-green-100/50 text-green-600 rounded-lg"><Percent size={24} /></div>
            </div>

            {/* Card 4: Custos de Viagem (Só Diretoria) */}
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 flex items-center justify-between">
              <div className="space-y-2">
                <p className="text-sm font-semibold text-slate-500 uppercase">Custos Viagem Internos</p>
                <h3 className="text-2xl font-bold text-red-600">{formatCurrency(data?.custosAcumuladosViagem)}</h3>
              </div>
              <div className="p-4 bg-red-50 text-red-600 rounded-lg"><Plane size={24} /></div>
            </div>
          </>
        ) : (
          ...
          <div className="lg:col-span-2 bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100 flex items-center justify-center">
            <p className="text-sm font-medium text-blue-700 text-center">🔒 Indicadores internos de rentabilidade e custos restritos à gestão RC.</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`bg-white p-6 rounded-xl shadow-md border border-slate-200 lg:col-span-2 space-y-6 ${userRole !== 'DIRETORIA' && 'opacity-40 pointer-events-none'}`}>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm font-medium">
              <span className="text-green-600">Faturado ({faturadoPercent.toFixed(1)}%)</span>
              <span className="text-amber-600">Pendente</span>
            </div>
            <div className="w-full bg-slate-100 h-5 rounded-full overflow-hidden flex border">
              <div style={{ width: `${faturadoPercent}%` }} className="bg-green-500 h-full"></div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t text-sm">
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase">Realizado</p>
              <p className="font-bold text-slate-700">{userRole === 'DIRETORIA' ? formatCurrency(data?.valorFaturado) : '---'}</p>
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase">A Receber</p>
              <p className="font-bold text-slate-700">{userRole === 'DIRETORIA' ? formatCurrency(data?.valorPendenteFaturamento) : '---'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200 space-y-6 flex flex-col justify-between">
          <div>
            <h2 className="text-lg font-bold text-slate-800 mb-4">Status de Entregas & SLA</h2>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-slate-50 border rounded-lg">
                ...
                <span className="text-lg font-bold text-slate-800">{data?.totalComarcasConcluidas}</span>
              </div>
          ...
          <button onClick={fetchDashboardData} className="w-full mt-4 py-2.5 bg-slate-700 hover:bg-slate-800 text-white font-semibold text-sm rounded-lg transition-colors">
            Atualizar Painel
          </button>
        </div>
